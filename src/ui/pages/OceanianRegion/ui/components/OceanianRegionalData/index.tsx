...
export const OceanianRegionData = () => {
  const { error, loading, regionData } = useRegionData("Oceania");
    ...
    >
      {regionData &&
        regionData.map((country) => (
          <Box key={country.alpha3Code} sx={{ cursor: "pointer" }}>
            <Card
              sx={{
              ...
              component="article"
              onClick={() => handleCardClick("Oceania", country)} // Add click handler here
            >
              <CardContent>
                {country.flag && (
                  <Image
